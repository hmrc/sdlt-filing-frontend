...
class RegistrationNumberFormProviderSpec extends StringFieldBehaviours {
  ...
  val exactLength = 9
  val invalidKey = "purchaser.registrationNumber.form.regex.error"
  ...
  ".value" - {
    ...
    "bind successfully when exactly 9 digits are present" - {
      // Valid bindings: exactly 9 digits
      behave like fieldThatBindsValidData(
        form,
        fieldName,
        numericStringsOfExactLengthNine
      )
    }

    "bind successfully when leading zeros are present" in {
      val result = form.bind(Map(fieldName -> "000000001"))
      result.errors mustBe Nil
      result.value mustBe Some("000000001")
    }
    ...
    "fail with regex error for non-numeric input (ignoring args)" in {
      ...
      badValues.foreach { v =>
        val result = form.bind(Map(fieldName -> v))
        result.errors.map(_.message) must contain only "purchaser.registrationNumber.form.regex.error"
      }
    }
  }
  }
