...
class TransactionVatAmountFormProviderSpec extends StringFieldBehaviours {

  val requiredKey = "transaction.vatAmount.error.required"
  val invalidNumericKey = "transaction.vatAmount.error.invalidNumeric"
  val invalidWholeNumberKey = "transaction.vatAmount.error.invalidWholeNumber"
  val maxValueKey = "transaction.vatAmount.error.maximum"

  val form = new TransactionVatAmountFormProvider()(_ => true)
  val formWithInvalidVat = new TransactionVatAmountFormProvider()(_ => false)
  ...
  ".value" - {
    ...
    "must fail with vat included in total consideration error when the predicate is false" in {
      ...
      result.errors must contain only FormError(fieldName, "transaction.vatAmount.error.vatIncludedInTotalConsideration", Seq.empty)
    }
  }
}
