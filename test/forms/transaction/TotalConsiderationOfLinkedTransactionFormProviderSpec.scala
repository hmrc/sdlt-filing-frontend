...
class TotalConsiderationOfLinkedTransactionFormProviderSpec extends StringFieldBehaviours {

  val requiredKey = "transaction.totalConsiderationOfLinkedTransaction.error.required"
  val invalidNumericKey = "transaction.totalConsiderationOfLinkedTransaction.error.invalidNumeric"
  val invalidWholeNumberKey = "transaction.totalConsiderationOfLinkedTransaction.error.invalidWholeNumber"
  val maxValueKey = "transaction.totalConsiderationOfLinkedTransaction.error.maximum"

  val form = new TotalConsiderationOfLinkedTransactionFormProvider()(_ => true)
  val formWithInvalidTotalLinkedConsideration = new TotalConsiderationOfLinkedTransactionFormProvider()(_ => false)
  ...
  ".value" - {
    ...
    "must fail with total consideration error when the predicate is false" in {
      ...
      result.errors must contain only FormError(fieldName, "transaction.totalConsiderationOfLinkedTransaction.error.totalConsideration", Seq.empty)
    }
  }
}
