...
class LandTypeOfPropertyControllerSpec extends SpecBase with MockitoSugar {
  ...
  "LandTypeOfProperty Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val application = applicationBuilder(userAnswers = Some(userAnswersResidential)).build()
      ...
      running(application) {
        val request =
          FakeRequest(POST, landTypeOfPropertyRoute)
            .withFormUrlEncodedBody(("value", LandTypeOfProperty.values.head.toString))
