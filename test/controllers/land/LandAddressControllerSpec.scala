...
class LandAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val redirectToAddressLookupRoute = controllers.land.routes.LandAddressController.redirectToAddressLookupLand().url
  lazy val redirectToAddressLookupChangeRoute = controllers.land.routes.LandAddressController.redirectToAddressLookupLand(changeRoute = Some("change")).url
  lazy val addressLookupCallbackRoute = controllers.land.routes.LandAddressController.addressLookupCallbackLand(id = "test-id").url
  lazy val addressLookupCallbackChangeRoute = controllers.land.routes.LandAddressController.addressLookupCallbackChangeLand("test-id").url
  ...
  val testUserAnswersMissingLandAddress = UserAnswers(
    id = "test-session-id",
    storn = "test-storn-123",
    returnId = Some("test-return-id"),
    fullReturn = None,
  ...
  "LandAddressController" - {

    "redirectToAddressLookupLand" - {
      ...
      "must redirect to address lookup service when no changeRoute is provided" in {
        ...
        running(application) {
          ...
          verify(mockAddressLookupService, times(1))
            .getJourneyUrl(any(), eqTo(controllers.land.routes.LandAddressController.addressLookupCallbackLand()), eqTo(true), any(), any())(any(), any(), any())
        }
      }
      ...
      "must redirect to address lookup service with change callback when changeRoute is provided" in {
        ...
        running(application) {
          ...
          verify(mockAddressLookupService, times(1))
            .getJourneyUrl(any(), eqTo(controllers.land.routes.LandAddressController.addressLookupCallbackChangeLand()), eqTo(true), any(), any())(any(), any(), any())
        }
      }
    ...
    "addressLookupCallbackLand" - {
      ...
      "must handle different address IDs" in {
        ...
        addressIds.foreach { addressId =>
          ...
          running(application) {
            val callbackRoute = controllers.land.routes.LandAddressController.addressLookupCallbackLand(addressId).url
      ...
      "addressLookupCallbackLand" - {
        ...
        "must redirect to ReturnTaskList when address is successfully saved" in { // TODO Change this when we have the check your answers page
          val mockAddressLookupService = mock[AddressLookupService]
        ...
        "must handle different address IDs" in {
          ...
          addressIds.foreach { addressId =>
            ...
            running(application) {
              ...
              status(result) mustEqual SEE_OTHER
              // TODO: change this when we have the check your answers page
              redirectLocation(result).value mustEqual controllers.routes.ReturnTaskListController.onPageLoad().url
