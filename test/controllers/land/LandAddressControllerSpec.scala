...
class LandAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "LandAddressController" - {
    ...
    "addressLookupCallbackChangeLand" - {
      ...
      "must redirect to Land CYA when address is successfully saved" in {
        val mockAddressLookupService = mock[AddressLookupService]
      ...
      "must handle different address IDs" in {
        ...
        addressIds.foreach { addressId =>
          ...
          running(application) {
            val callbackChangeRoute = controllers.land.routes.LandAddressController.addressLookupCallbackChangeLand(addressId).url
