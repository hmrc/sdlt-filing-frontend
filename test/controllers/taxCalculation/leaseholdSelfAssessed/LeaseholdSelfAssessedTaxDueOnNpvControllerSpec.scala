...
class LeaseholdSelfAssessedTaxDueOnNpvControllerSpec extends SpecBase with MockitoSugar {
  ...
  "LeaseholdSelfAssessedTaxDueOnNpv Controller" - {
    ...
    "must redirect to total amount due page when valid data is submitted in normal mode" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual controllers.routes.IndexController.onPageLoad().url // TODO: Update to correct controller
      }
    }
    
    // TODO: add check mode test once CYA is built
