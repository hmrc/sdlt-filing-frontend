...
class LeaseholdSelfAssessedBYSControllerSpec extends SpecBase {

  "LeaseHoldSelfAssessedBYS Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, controllers.taxCalculation.leaseholdSelfAssessed.routes.LeaseholdSelfAssessedBYSController.onPageLoad().url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[LeaseholdSelfAssessedBYSView]
