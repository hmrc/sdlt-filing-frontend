...
class TaxCalculationCheckYourAnswersControllerSpec extends SpecBase with MockitoSugar {
  ...
  // A complete return so the calculation request can be built for the calculated flow.
  ...
  private def appWith(answers: UserAnswers, calcResponse: Future[CalculationResponse] = Future.successful(CalculationResponse(Seq.empty))) = {
  ...
  private def getResult(app: play.api.Application) =
  ...
  "TaxCalculationCheckYourAnswersController" - {
    ...
    "onPageLoad" - {
      ...
      "returns OK for a calculated flow when the calculation succeeds" in {
        ...
        running(app) {
          status(getResult(app)) mustEqual OK
        }
      }
