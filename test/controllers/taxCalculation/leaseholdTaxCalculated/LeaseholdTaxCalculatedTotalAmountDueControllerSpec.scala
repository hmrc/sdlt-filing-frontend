...
class LeaseholdTaxCalculatedTotalAmountDueControllerSpec extends SpecBase with MockitoSugar {
  ...
  "LeaseholdTaxCalculatedTotalAmountDueController" - {
    ...
    "must return OK and render the total-amount-due view when sdltc and the helper both succeed" in {
      ...
      running(app) {
        val request = FakeRequest(GET, routes.LeaseholdTaxCalculatedTotalAmountDueController.onPageLoad(NormalMode).url)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val previouslyAnswered = leaseholdAnswers.set(LeaseholdTaxCalculatedTotalAmountDuePage, "12345").success.value
