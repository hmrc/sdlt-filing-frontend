...
class WhoAreYouSubmittingForControllerSpec extends SpecBase with MockitoSugar {
  ...
  "WhoAreYouSubmittingFor Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[WhoAreYouSubmittingForView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(WhoAreYouSubmittingFor.values.head), NormalMode)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, whoAreYouSubmittingForRoute)
