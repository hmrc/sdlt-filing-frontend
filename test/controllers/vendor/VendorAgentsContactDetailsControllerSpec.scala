...
class VendorAgentsContactDetailsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "VendorAgentsContactDetails Controller" - {
    ...
    "must redirect to ReturnTaskList if knowsAgentDetails is false" in {
      ...
      running(application) {
        ...
        val controller = new VendorAgentsContactDetailsController(
          messagesApi = application.injector.instanceOf[MessagesApi],
          sessionRepository = application.injector.instanceOf[SessionRepository],
          navigator = application.injector.instanceOf[Navigator],
          ...
          formProvider = new VendorAgentsContactDetailsFormProvider(),
          controllerComponents = application.injector.instanceOf[MessagesControllerComponents],
          agentChecksService = application.injector.instanceOf[AgentChecksService],
          view = application.injector.instanceOf[VendorAgentsContactDetailsView]
        ){
          override protected def tempKnowsAgentDetails(ua: UserAnswers): Boolean = false
        }
