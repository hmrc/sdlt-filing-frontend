...
class VendorAgentCheckYourAnswersControllerSpec extends SpecBase with SummaryListFluency with MockitoSugar with BeforeAndAfterEach {
  ...
  "VendorAgentCheckYourAnswers Controller" - {
    ...
    "onPageLoad" - {
      ...
      "must return OK and the correct view for a GET" in {
        ...
        when(mockSessionRepository.get(any()))
          .thenReturn(Future.successful(Some(userAnswersWithCompleteAnswersAndReturnAgentId.set(AgentNamePage, "Agent name").success.value)))

        val application = applicationBuilder(userAnswers = Some(userAnswersWithCompleteAnswersAndReturnAgentId))
    ...
    "onSubmit" - {
      ...
      "must redirect back to check your answers when updateReturnVersion returns no new version" in {
        val userAnswers = userAnswersWithCompleteAnswersAndReturnAgentId
          .set(VendorAgentOverviewPage, "AGENT123").success.value
