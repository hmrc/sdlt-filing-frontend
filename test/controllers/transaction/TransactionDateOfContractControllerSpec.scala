...
class TransactionDateOfContractControllerSpec extends SpecBase with MockitoSugar {

  private implicit val messages: Messages = stubMessages()

  private val time = TimeMachine()
  private val formProvider = new TransactionDateOfContractFormProvider(time)
  ...
  def postRequest(): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, transactionDateOfContractRoute)
  ...
  "TransactionDateOfContract Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[TransactionDateOfContractView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(TransactionDateOfContractPage, validAnswer).success.value
    ...
    "must redirect to the is transaction linked to another page when valid data is submitted and transaction type is grant of lease" in {
    ...
    "must redirect to the next page when valid data is submitted and transaction type is other than grant of lease" in {
    ...
    "must redirect to the type of transaction page when valid data is submitted and transaction type is not set" in {
