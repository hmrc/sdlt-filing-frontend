...
class PurchaserEligibleToClaimReliefControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val purchaserEligibleToClaimReliefRouteCheckMode: String = controllers.transaction.routes.PurchaserEligibleToClaimReliefController.onPageLoad(CheckMode).url
  ...
  "PurchaserEligibleToClaimRelief Controller" - {
    ...
    "must redirect to the Transaction CYA page when No is submitted in check mode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, purchaserEligibleToClaimReliefRouteCheckMode)
            .withFormUrlEncodedBody(("value", "false"))
