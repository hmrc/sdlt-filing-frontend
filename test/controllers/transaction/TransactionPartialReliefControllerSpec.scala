...
class TransactionPartialReliefControllerSpec extends SpecBase with MockitoSugar {
  ...
  "TransactionPartialRelief Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, transactionPartialReliefRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[TransactionPartialReliefView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(TransactionPartialReliefPage, true).success.value
    ...
    "must redirect to the next page (tr-8e) when 'Yes' is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, transactionPartialReliefRoute)
    ...
    "must redirect to considerations affected by uncertain future events page when 'No' is submitted" in {
