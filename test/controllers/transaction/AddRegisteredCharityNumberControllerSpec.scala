...
import views.html.transaction.AddRegisteredCharityNumberView

import scala.concurrent.Future

class AddRegisteredCharityNumberControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new AddRegisteredCharityNumberFormProvider()
  val form = formProvider()

  lazy val addRegisteredCharityNumberRoute = controllers.transaction.routes.AddRegisteredCharityNumberController.onPageLoad(NormalMode).url
  
  lazy val addRegisteredCharityNumberRouteCheckMode = controllers.transaction.routes.AddRegisteredCharityNumberController.onPageLoad(CheckMode).url

  "AddRegisteredCharityNumber Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, addRegisteredCharityNumberRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[AddRegisteredCharityNumberView]
    ...
    "must redirect to the Transaction CYA page when No is submitted in check mode" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, addRegisteredCharityNumberRouteCheckMode)
            .withFormUrlEncodedBody(("value", "false"))
