...
class TransactionAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val redirectToAddressLookupRoute = controllers.transaction.routes.TransactionAddressController.redirectToAddressLookupTransaction().url
  lazy val redirectToAddressLookupChangeRoute = controllers.transaction.routes.TransactionAddressController.redirectToAddressLookupTransaction(changeRoute = Some("change")).url
  lazy val addressLookupCallbackRoute = controllers.transaction.routes.TransactionAddressController.addressLookupCallbackTransaction(id = "test-id").url
  lazy val addressLookupCallbackChangeRoute = controllers.transaction.routes.TransactionAddressController.addressLookupCallbackChangeTransaction("test-id").url

  val testUserAnswersMissingTransactionAddress = UserAnswers(
    id = "test-session-id",
  ...
  "TransactionAddressController" - {

    "redirectToAddressLookupTransaction" - {
      ...
      "must redirect to address lookup service when no changeRoute is provided" in {
        ...
        running(application) {
          ...
          verify(mockAddressLookupService, times(1))
            .getJourneyUrl(any(), eqTo(controllers.transaction.routes.TransactionAddressController.addressLookupCallbackTransaction()), eqTo(true), any(), any())(any(), any(), any())
        }
      }
      ...
      "must redirect to address lookup service with change callback when changeRoute is provided" in {
        ...
        running(application) {
          ...
          verify(mockAddressLookupService, times(1))
            .getJourneyUrl(any(), eqTo(controllers.transaction.routes.TransactionAddressController.addressLookupCallbackChangeTransaction()), eqTo(true), any(), any())(any(), any(), any())
        }
      }
    ...
    "addressLookupCallbackTransaction" - {
      ...
      "must handle different address IDs" in {
        ...
        addressIds.foreach { addressId =>
          ...
          running(application) {
            val callbackRoute = controllers.transaction.routes.TransactionAddressController.addressLookupCallbackTransaction(addressId).url
    ...
    "addressLookupCallbackChangeTransaction" - {

      "must redirect to Transaction CYA when address is successfully saved" in {
        val mockAddressLookupService = mock[AddressLookupService]
      ...
      "must handle different address IDs" in {
        ...
        addressIds.foreach { addressId =>
          ...
          running(application) {
            val callbackChangeRoute = controllers.transaction.routes.TransactionAddressController.addressLookupCallbackChangeTransaction(addressId).url
