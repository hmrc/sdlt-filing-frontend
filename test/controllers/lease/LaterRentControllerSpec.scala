...
class LaterRentControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val laterRentRoute: String = controllers.lease.routes.LaterRentController.onPageLoad(NormalMode).url
  ...
  "LaterRent Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered and when transaction type is L - Grant of Lease" in {
      ...
      val userAnswers = userAnswersGrantOfLease.set(LaterRentPage, true).success.value
