...
class PurchaserDateOfBirthControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val purchaserDateOfBirthRoute = controllers.purchaser.routes.PurchaserDateOfBirthController.onPageLoad(NormalMode).url

  def getRequest(): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, purchaserDateOfBirthRoute)

  def postRequest(): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, purchaserDateOfBirthRoute)
  ...
  val testUserAnswersWithNoPurchaserName: UserAnswers = UserAnswers(
    id = "test-session-id",
    storn = "test-storn-123",
    returnId = Some("test-return-id"),
    fullReturn = None,
  ...
  "PurchaserDateOfBirth Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(form, purchaserName, NormalMode)(getRequest(), messages(application)).toString
      }
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers = testUserAnswersIndividual.set(PurchaserDateOfBirthPage, validAnswer).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(validAnswer), purchaserName, NormalMode)(getRequest(), messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, purchaserName, NormalMode)(request, messages(application)).toString
      }
    }
