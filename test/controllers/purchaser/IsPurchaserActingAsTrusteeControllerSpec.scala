...
class IsPurchaserActingAsTrusteeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IsPurchaserActingAsTrustee Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val purchaserName = NameOfPurchaser(Some("John"), Some("Doe"), "John Doe")

      val userAnswers = emptyUserAnswers.set(NameOfPurchaserPage, purchaserName).success.value

      val application = applicationBuilder(userAnswers = Some(userAnswers)).build()

      running(application) {
        val request = FakeRequest(GET, isPurchaserActingAsTrusteeRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[IsPurchaserActingAsTrusteeView]

        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form, NormalMode, purchaserName.fullName)(request, messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, NormalMode, purchaserName.fullName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to NameOfPurchaserController for a GET if no purchaser name is found" in {
    ...
    "must redirect to NameOfPurchaserController for a POST if no purchaser name is found" in {
