...
class IsPurchaserIndividualPageSpec extends AnyWordSpec with ScalaSpecBase with TestObjects {
  "cleanup" must {
    "must cleanup correct fields" when {
      "the answer is No" in {
        val userAnswers = emptyUserAnswers.copy(data = freeResNonIndAddMainFullUaData)
        val updatedAnswer = userAnswers.set(IsPurchaserIndividualPage, false).success.value
