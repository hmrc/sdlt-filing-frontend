...
class CalculationResultViewModelSpec extends SpecBase with EitherValues {
  ...
  ".toViewModel" - {
    ...
    "Left(MissingMainLandIdError) when ReturnInfo.mainLandID is not set" in {
      val noMainId = freeholdAnswers.copy(fullReturn = freeholdAnswers.fullReturn.map(fr =>
        fr.copy(returnInfo = Some(ReturnInfo()))
      ))
      toViewModel(freeholdResult, noMainId) mustBe Left(MissingMainLandIdError)
    }

    "Left(MissingAboutTheLandError) when no land in the list matches mainLandID" in {
      val noMatch = freeholdAnswers.copy(fullReturn = freeholdAnswers.fullReturn.map(fr =>
        fr.copy(
          returnInfo = Some(ReturnInfo(mainLandID = Some("L99"))),
          land       = Some(Seq(Land(landID = Some("L1"), propertyType = Some("R"))))
        )
      ))
      toViewModel(freeholdResult, noMatch) mustBe Left(MissingAboutTheLandError)
    }

    "selects the main land by mainLandID, not by position in the list" in {
      val notMain = Land(landID = Some("L1"), propertyType = Some("R"))
      val main    = Land(landID = Some("L2"), propertyType = Some("M"))
      val pickedById = freeholdAnswers.copy(fullReturn = freeholdAnswers.fullReturn.map(fr =>
        fr.copy(
          returnInfo = Some(ReturnInfo(mainLandID = Some("L2"))),
          land       = Some(Seq(notMain, main))
        )
      ))
      toViewModel(freeholdResult, pickedById).map { vm =>
        vm.rateCardSummary.rows.exists(_.value.content == Text("taxCalculation.calculation.propertyType.M"))
      } mustBe Right(true)
    }
    ...
    "Left(MissingLandAnswerError) when land has no propertyType" in {
      val missingPropType = freeholdAnswers.copy(fullReturn = freeholdAnswers.fullReturn.map(fr =>
        fr.copy(land = Some(Seq(Land(landID = Some("L1"), propertyType = None))))
      ))
