    ...
    "when address data is present" - {
      ...
      "must return a summary list row with formatted address and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.href mustEqual controllers.land.routes.LandAddressController.redirectToAddressLookupLand(Some("change")).url
          result.actions.get.items.head.content.asHtml.toString() must include(msgs("site.change"))
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("land.address.change.hidden")
        }
      }
