...
class DoYouKnowTheAreaOfLandSummarySpec extends SpecBase {
  ...
  "DoYouKnowTheAreaOfLandSummary" - {

    "when do you know the area of land is present" - {
      ...
      "must return a SummaryListRow with 'yes' text and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.href mustEqual controllers.land.routes.DoYouKnowTheAreaOfLandController.onPageLoad(CheckMode).url
          result.actions.get.items.head.content.asHtml.toString() must include(msgs("site.change"))
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("land.doYouKnowTheAreaOfLand.change.hidden")
        }
      }
