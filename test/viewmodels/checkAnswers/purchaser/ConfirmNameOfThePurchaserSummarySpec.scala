...
class ConfirmNameOfThePurchaserSummarySpec extends SpecBase {
  ...
  "ConfirmNameOfThePurchaserSummary" - {

    "when confirmation is Yes" - {

      "must return a summary list row with Yes label" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("nameOfThePurchaser.change.hidden")
        }
      }
    }

    "when confirmation is No" - {

      "must return a summary list row with No label" in {
        val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
        running(application) {
          implicit val msgs: Messages = messages(application)
    ...
    "when confirmation is not present" - {
