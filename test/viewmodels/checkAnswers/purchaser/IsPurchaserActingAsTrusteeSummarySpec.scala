...
class IsPurchaserActingAsTrusteeSummarySpec extends SpecBase {
  ...
  "IsPurchaserActingAsTrusteeSummary" - {
    ...
    "must return a SummaryListRow when IsPurchaserActingAsTrusteePage is set and change link" in {
        val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()

        running(application) {
          implicit val msgs: Messages = messages(application)

          val userAnswers = emptyUserAnswers.set(IsPurchaserActingAsTrusteePage, IsPurchaserActingAsTrustee.Yes).success.value

          val result = IsPurchaserActingAsTrusteeSummary.row(userAnswers).getOrElse(fail("Failed to get summary list row"))

          result.key.content.asHtml.toString() mustEqual msgs("purchaser.isPurchaserActingAsTrustee.checkYourAnswersLabel")

          val contentString = result.value.content.asHtml.toString()

          contentString mustEqual msgs(s"purchaser.isPurchaserActingAsTrustee.${IsPurchaserActingAsTrustee.Yes}")
    ...
    "must return None when AddVendorAgentContactDetailsPage is not answered" in {
      ...
      running(application) {
        ...
        IsPurchaserActingAsTrusteeSummary.row(emptyUserAnswers) mustBe None
      }
    }
  }
}
