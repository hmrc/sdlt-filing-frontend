...
class PurchaserConfirmIdentitySummarySpec extends SpecBase {
  ...
  "PurchaserConfirmIdentitySummary" - {
    ...
    "must return a SummaryListRow with PurchaserConfirmIdentityPage is set with a change link" in {
      ...
      running(application) {
        ...
        val result = PurchaserConfirmIdentitySummary.row(userAnswers).getOrElse(fail("Failed to get summary list row"))
        result.key.content.asHtml.toString() mustEqual msgs("purchaser.confirmIdentity.checkYourAnswersLabel")

        val contentString = result.value.content.asInstanceOf[HtmlContent].toString
        contentString must include(msgs(s"purchaser.confirmIdentity.${PurchaserConfirmIdentity.PartnershipUTR}"))
    ...
    "must return None when AddVendorAgentContactDetailsPage is not answered" in {
      ...
      running(application) {
        ...
        PurchaserConfirmIdentitySummary.row(emptyUserAnswers) mustBe None
      }
    }
  }
}