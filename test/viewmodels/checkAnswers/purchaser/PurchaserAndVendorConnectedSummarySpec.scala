...
import models.purchaser.{PurchaserAndVendorConnected, NameOfPurchaser}
    ...
    "when purchaser name is present" - {
      ...
      "must return a summary list row with surname only" in {
        ...
        running(application) {
          ...
          val result = PurchaserAndVendorConnectedSummary.row(userAnswers).getOrElse(fail("Failed to get summary list row"))

          result.key.content.asHtml.toString() mustEqual msgs("purchaser.purchaserAndVendorConnected.checkYourAnswersLabel")
