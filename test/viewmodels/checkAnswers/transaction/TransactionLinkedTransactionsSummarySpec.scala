...
class TransactionLinkedTransactionsSummarySpec extends SpecBase {
  ...
  "TransactionLinkedTransactionsSummary" - {
    ...
    "when answer is present" - {
      ...
      "must return a summary list row with YES value and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("transaction.linkedTransactions.change.hidden")
        }
      }
