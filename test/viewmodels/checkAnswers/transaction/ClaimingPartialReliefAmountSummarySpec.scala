...
class ClaimingPartialReliefAmountSummarySpec extends SpecBase {
  ...
  "ClaimingPartialReliefAmountSummary" - {

    "when the partial relief amount is present" - {
      ...
      "must return a summary list row with value and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.href mustEqual controllers.transaction.routes.ClaimingPartialReliefAmountController.onPageLoad(CheckMode).url
          result.actions.get.items.head.content.asHtml.toString() must include(msgs("site.change"))
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("transaction.claimingPartialReliefAmount.change.hidden")
        }
      }
    }
