  ...
  "TransactionFormsOfConsiderationSummary" - {
    ...
    "must return a summary list row with surname only" in {
        ...
        running(application) {
          ...
          htmlContent mustEqual "Cash"

          result.actions.get.items.size mustEqual 1
          result.actions.get.items.head.href mustEqual controllers.transaction.routes.TransactionFormsOfConsiderationController.onPageLoad(CheckMode).url
