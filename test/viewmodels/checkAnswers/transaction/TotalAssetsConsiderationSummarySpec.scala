...
class TotalAssetsConsiderationSummarySpec  extends SpecBase {
  ...
  "TotalAssetsConsiderationSummary" - {

    "when the total assets consideration amount is present" - {
      ...
      "must return a summary list row with value and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.href mustEqual controllers.transaction.routes.TotalAssetsConsiderationController.onPageLoad(CheckMode).url
          result.actions.get.items.head.content.asHtml.toString() must include(msgs("site.change"))
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("transaction.totalAssetsConsideration.change.hidden")
        }
      }
    }

    "when the total assets consideration amount is not present" - {
