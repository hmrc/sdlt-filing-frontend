...
class TotalConsiderationOfTransactionSummarySpec extends SpecBase {
  ...
  "TotalConsiderationOfTransactionSummary" - {
    ...
    "when the total consideration of transaction is present" - {
      ...
      "must return a summary list row with value and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.href mustEqual controllers.transaction.routes.TotalConsiderationOfTransactionController.onPageLoad(CheckMode).url
          result.actions.get.items.head.content.asHtml.toString() must include(msgs("site.change"))
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("transaction.totalConsiderationOfTransaction.change.hidden")
        }
      }
    }
