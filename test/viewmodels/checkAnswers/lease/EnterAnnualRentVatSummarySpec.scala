...
class EnterAnnualRentVatSummarySpec extends SpecBase {

  "EnterAnnualRentVatSummary" - {

    "when annual rent VAT is present  " - {
      ...
      "must return a SummaryListRow annual starting rent value and change link" in {
        ...
        running(application) {
          ...
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("lease.enterAnnualRentVat.change.hidden")
        }
      }
}
    ...
    "when annual starting rent is not present" - {
      ...
      "must return a Missing and redirect call to missing page when LeaseIsVatPayablePage is true but EnterAnnualRentVatPage is missing" in {
        ...
        running(application) {
          ...
          result match {
            case Missing(call) =>
              call mustEqual controllers.lease.routes.EnterAnnualRentVatController.onPageLoad(CheckMode)
