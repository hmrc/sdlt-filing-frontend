...
class LeaseholdTaxCalculatedSelfAssessedAmountSummarySpec extends SpecBase {

  "LeaseholdTaxCalculatedSelfAssessedAmountSummary" - {
    ...
    "when an answer has been entered" - {
      ...
      "must return a Row with the £-prefixed amount and a change link" in {
        ...
        running(app) {
          ...
          val changeRoute = controllers.taxCalculation.leaseholdTaxCalculated.routes.LeaseholdTaxCalculatedSdltSelfAssessmentController.onPageLoad(CheckMode)
          val answers     = emptyUserAnswers.set(LeaseholdTaxCalculatedSelfAssessedAmountPage, "12345").success.value

          val result = LeaseholdTaxCalculatedSelfAssessedAmountSummary.row(Some(answers)) match {
            case Row(r) => r
    ...
    "when no answer has been entered" - {
      ...
      "must return Missing pointing at the question page in CheckMode" in {
        ...
        running(app) {
          ...
          LeaseholdTaxCalculatedSelfAssessedAmountSummary.row(Some(emptyUserAnswers)) mustBe Missing(changeRoute)
        }
      }
      ...
      "must return Missing when UserAnswers is None" in {
        ...
        running(app) {
          ...
          LeaseholdTaxCalculatedSelfAssessedAmountSummary.row(None) mustBe Missing(changeRoute)
        }
      }
    }
  }
