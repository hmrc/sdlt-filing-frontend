...
class TaxDueOnNpvSummarySpec extends SpecBase {
      ...
      "must return a summary list row with value and change link" in {
        ...
        running(application) {
          ...
          val userAnswers = emptyUserAnswers
            .set(LeaseholdSelfAssessedNpvTaxPage, "100.00").success.value

          val row = TaxDueOnNpvSummary.row(userAnswers)
          ...
          result.actions.get.items.head.href mustEqual
            controllers.taxCalculation.leaseholdSelfAssessed.routes.LeaseholdSelfAssessedTaxDueOnNpvController.onPageLoad(CheckMode).url
          result.actions.get.items.head.content.asHtml.toString() must include(msgs("site.change"))
          result.actions.get.items.head.visuallyHiddenText.value mustEqual msgs("taxCalculation.taxDueOnNpv.change.hidden")
        }
      }
    }
    ...
    "when the tax due on NPV is not present" - {
      ...
      "must return a Missing and redirect call to missing page" in {
        ...
        running(application) {
          ...
          val result = TaxDueOnNpvSummary.row(emptyUserAnswers)

          result match {
            case Missing(call) =>
              call mustEqual controllers.taxCalculation.leaseholdSelfAssessed.routes.LeaseholdSelfAssessedTaxDueOnNpvController.onPageLoad(CheckMode)
