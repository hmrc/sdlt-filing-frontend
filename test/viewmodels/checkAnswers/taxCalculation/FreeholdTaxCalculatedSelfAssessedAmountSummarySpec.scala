...
class FreeholdTaxCalculatedSelfAssessedAmountSummarySpec extends SpecBase {

  "FreeholdTaxCalculatedSelfAssessedAmountSummary" - {
    ...
    "when an answer has been entered" - {
      ...
      "must return a Row with the £-prefixed amount and a change link" in {
        ...
        running(app) {
          ...
          val changeRoute = controllers.taxCalculation.freeholdTaxCalculated.routes.FreeholdTaxCalculatedSdltSelfAssessmentController.onPageLoad(CheckMode)
          val answers     = emptyUserAnswers.set(FreeholdTaxCalculatedSelfAssessedAmountPage, "12345").success.value

          val result = FreeholdTaxCalculatedSelfAssessedAmountSummary.row(Some(answers)) match {
            case Row(r) => r
    ...
    "when no answer has been entered" - {
      ...
      "must return Missing pointing at the question page in CheckMode" in {
        ...
        running(app) {
          ...
          FreeholdTaxCalculatedSelfAssessedAmountSummary.row(Some(emptyUserAnswers)) mustBe Missing(changeRoute)
        }
      }
      ...
      "must return Missing when UserAnswers is None" in {
        ...
        running(app) {
          ...
          FreeholdTaxCalculatedSelfAssessedAmountSummary.row(None) mustBe Missing(changeRoute)
        }
      }
    }
  }
