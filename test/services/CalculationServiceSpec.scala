...
class CalculationServiceSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach
  ...
  "CalculateTax" must {
    ...
    def createRequest(hType: HoldingTypes.Value, pType: PropertyTypes.Value, eDate: LocalDate, taxReliefCode: Option[TaxReliefCode] = None,
                      isLinked: Option[Boolean] = None, isAddProp: Boolean = false, isPartialRelief: Option[Boolean] = None,
                      isMultipleLand: Option[Boolean] = None, relevantRent:Option[BigDecimal]=None,
                      leaseDetails:Option[LeaseDetails] = None, declaredNpv: Option[BigDecimal] = None) =  Request(
      holdingType = hType,
      propertyType = pType,
      effectiveDate = eDate,
      nonUKResident = None,
    ...
    //SDLT - Tax Calc Case - 60 - self assessed
    "select the leaseholdResFTBAfterNov17AndBeforeJul20upTo500k function" when {
      "Leasehold & Residential & FirstTimeBuyersRelief & isLinked & premium <= 500000 & isMultipleLand = false & date is on or after 22/11/2017" in {
        val testRequest = createRequest(leasehold, residential, LocalDate.of(2017, 11, 22), Some(FirstTimeBuyersRelief), isLinked = Some(true), isMultipleLand = Some(false))
          .copy(premium = 500000)
      ...
      "Leasehold & Residential & FirstTimeBuyersRelief & isLinked & premium <= 500000 & isMultipleLand = false & date is before 8/7/2020" in {
      ...
      "Leasehold, residential, FirstTimeBuyersRelief, isLinked = true, premium > 500000 ,isMultipleLand = false and date is between 22/11/2017 and 08/07/2020" in {
        ...
        forAll(dateAfterNovember2017AndBeforeJuly2020) {
          date => {
            ...
            val result = createResult("leaseholdResFTBAfterNov17AndBeforeJul20greaterThan500k")

            reset(mockLeaseholdCalculationService)

            when(mockLeaseholdCalculationService.leaseholdResFTBAfterNov17AndBeforeJul20greaterThan500k).thenReturn(result)

            testCalculationService.calculateTax(testRequest).result shouldBe Seq(result)

            verify(mockLeaseholdCalculationService, times(1)).leaseholdResFTBAfterNov17AndBeforeJul20greaterThan500k
    ...
    "select the leaseholdReliefFrom15PercentRateMixedAndNonResAfterApril2013AndBeforeMarch2016 function" when {
      "LeaseHold, mixed, ReliefFrom15Percentage, isLinked = false, relevantRentDetails < 1000 and date is on 6thApril2013 " in {
        ...
        val result = createResult("leaseholdReliefFrom15PercentRateMixedAndNonResAfterApril2013AndBeforeMarch2016")

        when(mockLeaseholdCalculationService.leaseholdReliefFrom15PercentRateMixedAndNonResAfterApril2013AndBeforeMarch2016(any())).thenReturn(result)

        testCalculationService.calculateTax(testRequest).result mustBe Seq(result)

        verify(mockLeaseholdCalculationService, times(1)).leaseholdReliefFrom15PercentRateMixedAndNonResAfterApril2013AndBeforeMarch2016(any())
    ...
    "select the leaseholdMixedNonResBeforeMar08 function" when {
      "property type is Non-residential, effective date is before 12/3/2008, transaction is not linked, & relevant rent is >= 1000" in {
        val testRequest = createRequest(leasehold, nonResidential, LocalDate.of(2008, 3, 11), isLinked = Some(false)).copy(
        ...
        val result = createResult("leaseholdMixedNonResBeforeMar08")

        when(mockLeaseholdCalculationService.leaseholdMixedNonResBeforeMar08(any())).thenReturn(result)

        testCalculationService.calculateTax(testRequest) shouldBe CalculationResponse(Seq(result))

        verify(mockLeaseholdCalculationService, times(1)).leaseholdMixedNonResBeforeMar08(any())
        verifyNoMoreInteractions(mockLeaseholdCalculationService)
      }
      "property type is Mixed, effective date is before 12/3/2008, transaction is not linked, & relevant rent is >= 1000" in {
        val testRequest = createRequest(leasehold, mixed, LocalDate.of(2008, 3, 11), isLinked = Some(false)).copy(
          relevantRentDetails = Some(RelevantRentDetails(
            exchangedContractsBeforeMar16 = None,
            contractChangedSinceMar16 = None,
            relevantRent = Some(1001)
          ))
        )
    ...
    "select the leaseholdMixedNonResApr2013toMar2016" when {
      ...
      "given a request where property type is Non-residential, is before 17/03/2016, average rent > 1000 and isLinked = false" in {
        ...
        testCalculationService.calculateTax(nonResTestRequest) shouldBe CalculationResponse(Seq(result))
    ...
    "select leaseHoldResidentialFirstTimeBuyerReliefAfterNov2017AndBeforeJul20 function" when {
      "property type is Residential,LeaseHold, isMultipleLand = true, FirstTimeBuyersRelief , isLinked = false, and effective date is on 22ndNovember2017" in {
        val testRequest = createRequest(leasehold, residential, LocalDate.of(2017, 11, 22), Some(FirstTimeBuyersRelief), isLinked = Some(false), isMultipleLand = Some(true))

        val result = createResult("leaseHoldResidentialFirstTimeBuyerReliefAfterNov2017AndBeforeJul20")

        when(mockLeaseholdCalculationService.leaseHoldResidentialFirstTimeBuyerReliefAfterNov2017AndBeforeJul20(any())).thenReturn(result)

        testCalculationService.calculateTax(testRequest) shouldBe CalculationResponse(Seq(result))

        verify(mockLeaseholdCalculationService, times(1)).leaseHoldResidentialFirstTimeBuyerReliefAfterNov2017AndBeforeJul20(any())

        verifyNoMoreInteractions(mockLeaseholdCalculationService)
      }
      "property type is Residential,LeaseHold, isMultipleLand = true, FirstTimeBuyersRelief , isLinked = false, and effective date is on 11thMarch2019" in {
        val testRequest = createRequest(leasehold, residential, LocalDate.of(2019, 3, 11), Some(FirstTimeBuyersRelief), isLinked = Some(false), isMultipleLand = Some(true))
      ...
      "property type is Residential,LeaseHold, isMultipleLand = true, FirstTimeBuyersRelief , isLinked = false, and effective date is on 7thJuly2020" in {
