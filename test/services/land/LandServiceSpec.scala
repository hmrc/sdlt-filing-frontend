...
class LandServiceSpec extends SpecBase with Matchers {
  ...
  "propertyTypeCheckAsync" - {

    val continueRoute = Future.successful(Results.Ok("continue"))

    "must return continueRoute for Mixed" in {
      val userAnswers = emptyUserAnswers
        .set(LandTypeOfPropertyPage, LandTypeOfProperty.Mixed).success.value

      val futureResult = service.propertyTypeCheckAsync(userAnswers, continueRoute)

      whenReady(futureResult) { result =>
        result mustBe Results.Ok("continue")
      }
    }

    "must return continueRoute for NonResidential" in {
      val userAnswers = emptyUserAnswers
    ...
    "must redirect to LandTypeOfPropertyPage when property type is missing" in {
      val userAnswers = emptyUserAnswers
      ...
      redirectLocation(futureResult) mustBe Some(controllers.land.routes.LandTypeOfPropertyController.onPageLoad(NormalMode).url)
    }
