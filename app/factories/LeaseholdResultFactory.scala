...
object LeaseholdResultFactory {
  ...
  def leaseholdMixedNonResBeforeMar08Result(leaseResult: SliceResult, premiumResult: SlabResult, npv: BigDecimal): Result = {
    val leaseCalcDetails = CalculationDetails(
      taxType = TaxTypes.rent,
      calcType = CalcTypes.slice,
      detailHeading = None,
  ...
  def leaseholdReliefFrom15PercentRateMixedAndNonResAfterApril2013AndBeforeMarch2016Res(premiumResult: SlabResult, leaseResult: SliceResult, npv: BigDecimal): Result = {
    val premiumCalcDetails = CalculationDetails(
      taxType = TaxTypes.premium,
      calcType = CalcTypes.slab,
      detailHeading = None,
  ...
  def leaseholdReliefFrom15PercentRateAbove1kRentRes(premiumResult: SlabResult, leaseResult: SliceResult, npv: BigDecimal): Result = {
    val premiumCalcDetails = CalculationDetails(
      taxType = TaxTypes.premium,
      calcType = CalcTypes.slab,
      detailHeading = None,
  ...
  def leaseholdReliefFrom15PercentRateRightToBuyMixedOnOrAfterMarch2016Res(leaseResult: SliceResult, premiumResult: SliceResult, npv: BigDecimal): Result = {
    val leaseCalcDetails = CalculationDetails(
      taxType = TaxTypes.rent,
      calcType = CalcTypes.slice,
      detailHeading = Some(DETAIL_HEADING_SDLT_ON_RENT_FROM_MAR_2016),
