...
sealed trait ConfirmNameOfThePurchaser
...
object ConfirmNameOfThePurchaser extends Enumerable.Implicits {
  ...
  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"purchaser.nameOfThePurchaser.${value.toString}")),
        value   = Some(value.toString),
        id      = Some(s"value_$index")
      )
  }

  implicit val enumerable: Enumerable[ConfirmNameOfThePurchaser] =
