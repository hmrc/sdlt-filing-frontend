...
object Land {
  ...
  def from(userAnswers: UserAnswers): Future[Land] = {
    ...
    Future.successful(Land(
      ...
      areaUnit = landSessionQuestions.areaUnit,
      localAuthorityNumber = Some(landSessionQuestions.localAuthorityCode),
      mineralRights = if landSessionQuestions.landMineralsOrMineralRights then Some("yes") else Some("no"),
      NLPGUPRN = landSessionQuestions.landNlpgUprn,
      willSendPlanByPost = if landSessionQuestions.landSendingPlanByPost then Some("yes") else Some("no"),
...
object Transaction {
  ...
  def from(userAnswers: UserAnswers): Future[Transaction] = {
    ...
    Future.successful(Transaction(
      ...
      exchangedLandPostcode    = transactionSessionQuestions.transactionAddress.map(_.postcode),
