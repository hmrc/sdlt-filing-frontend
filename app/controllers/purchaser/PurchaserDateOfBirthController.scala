...
class PurchaserDateOfBirthController @Inject()(
      ...
      request.userAnswers.get(NameOfPurchaserPage) match {
        ...
        case Some(value) =>
          ...
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PurchaserDateOfBirthPage, value))
                _              <- sessionRepository.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(PurchaserDateOfBirthPage, mode, updatedAnswers))
          )
      }
  }
}
