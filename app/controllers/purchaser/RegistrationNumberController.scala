...
class RegistrationNumberController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      // ToDo  isPurchaserVATRegistered Page - hardcode value
      val isPurchaserVATRegistered: Option[String] = Some("VATRegistrationNumber")

      (purchaserFullName, isPurchaserVATRegistered) match
         case (Some(purchaserFullName), Some(isPurchaserVATRegistered))
              if(isPurchaserVATRegistered.equals("VATRegistrationNumber")) =>
    ...
    //todo to revert bcck to previous page
         case (_,_) => Redirect(controllers.routes.ReturnTaskListController.onPageLoad())
  }
