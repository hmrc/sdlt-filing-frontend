...
import pages.transaction.{SaleOfBusinessPage, TotalAssetsConsiderationPage, TransactionSaleOfBusinessAssetsPage}
        ...
        value =>
          for {
            ...
            finalAnswers   <- Future.fromTry {
              if !value then updatedAnswers.remove(TransactionSaleOfBusinessAssetsPage).flatMap(_.remove(TotalAssetsConsiderationPage))
