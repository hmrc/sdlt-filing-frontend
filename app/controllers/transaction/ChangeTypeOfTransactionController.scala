...
class ChangeTypeOfTransactionController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val option = generateOption(transactionType, typeOfTransactionPage)

      if (option == "Redirect") {
        Redirect(controllers.transaction.routes.TransactionEffectiveDateController.onPageLoad(NormalMode))
      } else {
        Ok(view(preparedForm, mode, option))
      }
  }
  ...
  private def generateOption(transactionType: Option[TransactionType], typeOfTransactionPage: Option[TransactionType]): String =
