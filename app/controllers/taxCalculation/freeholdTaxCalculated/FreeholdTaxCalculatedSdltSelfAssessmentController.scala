...
@Singleton
class FreeholdTaxCalculatedSdltSelfAssessmentController @Inject()(
        ...
        sdltCalculationService.calculateStampDutyLandTax(request.userAnswers).flatMap {
          case Right(_) =>
            form.bindFromRequest().fold(
              ...
              value =>
                for {
                  updated <- Future.fromTry(request.userAnswers.set(FreeholdTaxCalculatedSelfAssessedAmountPage, value))
                  _       <- sessionRepository.set(updated)
                } yield Redirect(navigator.nextPage(FreeholdTaxCalculatedSelfAssessedAmountPage, mode, updated))
            )
          case Left(err) =>
            logger.warn(s"[FreeholdTaxCalculatedSdltSelfAssessmentController][onSubmit] sdltc failed: ${err.message}")
