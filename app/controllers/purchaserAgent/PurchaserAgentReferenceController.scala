...
class PurchaserAgentReferenceController @Inject()(
      ...
      request.userAnswers.get(PurchaserAgentNamePage) match {
        ...
        case Some(agentName) =>
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PurchaserAgentReferencePage, value))
                _ <- sessionRepository.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(PurchaserAgentReferencePage, mode, updatedAnswers))
          )
      }
  }
}
