...
class PurchaserAgentAuthorisedController @Inject()(
      ...
      request.userAnswers.get(PurchaserAgentNamePage) match {
        ...
        case Some(agentName) =>
          form.bindFromRequest().fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(PurchaserAgentAuthorisedPage, value))
                _ <- sessionRepository.set(updatedAnswers)
              } yield {
                  Redirect(navigator.nextPage(PurchaserAgentAuthorisedPage, mode, updatedAnswers))
              }
          )
      }
  }
