...
class LocalAuthorityCodeController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val transactionDates: Option[(String, String)] = for {
        fr <- userAnswers.fullReturn
        tr <- fr.transaction
        effectDate <- tr.effectiveDate
        contractDate <- tr.contractDate} yield (effectDate, contractDate)

      val effectiveTransactionDate: Option[LocalDate] =
        transactionDates.map { case (eff, _) => LocalDate.parse(eff, formatter) }

      val contractEffDate: Option[LocalDate] = {
        transactionDates.map { case (_, con) => LocalDate.parse(con, formatter)
        }
      }
