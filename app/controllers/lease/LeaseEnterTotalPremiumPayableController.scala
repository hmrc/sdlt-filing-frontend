...
@Singleton
class LeaseEnterTotalPremiumPayableController @Inject()(
                                        ...
                                        formProvider: LeaseEnterTotalPremiumPayableFormProvider,
                                        leaseService: LeaseService,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: LeaseEnterTotalPremiumPayableView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData andThen statusCheck) {
    implicit request =>
      ...
      leaseService.leaseFlowValidationCheck(request.userAnswers) match {
        ...
        case None =>
          val preparedForm = request.userAnswers.get(LeaseEnterTotalPremiumPayablePage) match {
            case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData andThen statusCheck).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(LeaseEnterTotalPremiumPayablePage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(LeaseEnterTotalPremiumPayablePage, mode, updatedAnswers))
      )
  }
}
