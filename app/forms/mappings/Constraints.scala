...
trait Constraints {
  ...
  protected def maxLengthInt(maximum: Int, errorKey: String): Constraint[Int] =
    Constraint {
      case input if input.toString.length <= maximum =>
        Valid
      case _ =>
        Invalid(errorKey, maximum)
    }
  ...
  protected def localAuthorityCodeConstraints(
    ...
    invalidAreaPostcodeKey:   String,
    welshKey:                 String
  ): Constraint[String] = Constraint { code =>
    ...
    if (!code.matches("^[0-9]+$"))
      Invalid(invalidCharsKey)
    else if (!code.matches(authorityCodeRegex))
      Invalid(invalidFormatKey)
    else {
      val isWelsh = code.matches(welshLocalAuthNumbers)

      if (isWelsh && !checkWelshRegex(effectiveTransactionDate, code, contractEffDate, welshLocalAuthNumbers))
        Invalid(welshKey)
      else if (!isWelsh && !code.matches("^899[89]$") && isScottishPostcode(postcode))
        Invalid(invalidAreaPostcodeKey)
      else if (checkScotlandRegex(effectiveTransactionDate, contractEffDate, code, postcode) && !isWelsh)
        Valid
