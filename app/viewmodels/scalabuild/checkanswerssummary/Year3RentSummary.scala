...
object Year3RentSummary {
  ...
  def row(answers: UserAnswers, withAction: Boolean)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(RentPage).flatMap { answer =>
      answer.year3Rent.map { rent =>
        if (withAction) {
          SummaryListRowViewModel(
            key = KeyViewModel("year3Rent.checkYourAnswersLabel").withCssClass(keyCssClass),
            value = ValueViewModel.withId(text = bigDecimalFormat(rent), id = "td2_year3Rent"),
            actions = Seq(
              ActionItemViewModel(
                "site.change",
                controllers.scalabuild.routes.RentController.onPageLoad().url
              )
                .withVisuallyHiddenText(messages("site.change.hidden.year3Rent"))
                .withAttribute(("id", "change_year3Rent"))
            )
          )
        } else {
          SummaryListRowViewModel(
