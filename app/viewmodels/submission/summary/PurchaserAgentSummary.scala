...
object PurchaserAgentSummary {

  def getSummaryCard(fullReturn: FullReturn)(implicit messages: Messages): Option[SummaryList] = {
    val purchaserAgentOpt: Option[ReturnAgent] = fullReturn.returnAgent.flatMap(_.find(_.agentType.contains(AgentType.Purchaser.toString)))
    purchaserAgentOpt.flatMap { agent =>
      agent.name.map { agentName =>
        SummaryListViewModel(
          Seq(
            getOptSummaryRowHtml(
              messages("purchaserAgent.address.checkYourAnswersLabel"),
            ...
            getOptSummaryRowHtml(
              messages("purchaserAgent.contactDetails.checkYourAnswersLabel"),
            ...
            getOptSummaryRow(
              messages("purchaserAgent.reference.checkYourAnswersLabel"),
              agent.reference
            ),
            getOptSummaryRow(
              messages("purchaserAgent.purchaserAgentAuthorised.checkYourAnswersLabel", agentName),
              getOptYesNo(agent.isAuthorised)
            )
          ).flatMap(_.toSeq)
        ).withCard(
          messages(
            "submission.completedSdltReturn.purchaserAgent.header",
            agentName
          )
        )
      }
