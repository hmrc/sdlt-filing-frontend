...
object PurchaserAndVendorConnectedSummary  {
  ...
  def row(answers: Option[UserAnswers])(implicit messages: Messages): SummaryRowResult = {
    val changeRoute = controllers.purchaser.routes.PurchaserAndVendorConnectedController.onPageLoad(CheckMode)
          ...
          SummaryListRowViewModel(
            ...
            actions = Seq(
              ActionItemViewModel("site.change", changeRoute.url)
                .withVisuallyHiddenText(messages("purchaser.purchaserAndVendorConnected.change.hidden",answers.flatMap(_.get(NameOfPurchaserPage)).map(_.fullName).getOrElse("")))
            )
          )
        )
    }.getOrElse {
