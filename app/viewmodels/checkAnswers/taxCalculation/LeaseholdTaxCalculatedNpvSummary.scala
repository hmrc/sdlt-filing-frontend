...
object LeaseholdTaxCalculatedNpvSummary extends CurrencyFormatter {
  ...
  def row(result: TaxCalculationResult)(implicit messages: Messages): Option[SummaryRowResult] = {
    val label = messages("taxCalculation.leaseholdTaxCalculated.taxDueOnNpv.checkYourAnswers")

    result.taxCalcs.find(_.taxType == TaxTypes.rent).map { npv =>
      Row(
        SummaryListRowViewModel(
          key = label,
          value = ValueViewModel(HtmlContent(npv.taxDue.toCurrency))
        )
      )
    }
  }
