...
object FreeholdTaxCalculatedSelfAssessedAmountSummary extends CurrencyFormatter {
  ...
  def row(answers: Option[UserAnswers])(implicit messages: Messages): SummaryRowResult = {
    ...
    answers.flatMap(_.get(FreeholdTaxCalculatedSelfAssessedAmountPage)).map { answer =>
      Row(
