...
object LeaseholdTaxCalculatedPremiumPayableSummary extends CurrencyFormatter {
  ...
  def row(result: TaxCalculationResult)(implicit messages: Messages): Option[SummaryRowResult] = {
    val label = messages("taxCalculation.leaseholdTaxCalculated.premiumPayable.checkYourAnswers")

    result.taxCalcs.find(_.taxType == TaxTypes.premium).map { premium =>
      Row(
        SummaryListRowViewModel(
          key = label,
          value = ValueViewModel(HtmlContent(premium.taxDue.toCurrency))
        )
      )
    }
  }
