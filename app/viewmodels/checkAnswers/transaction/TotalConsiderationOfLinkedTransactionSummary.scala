    ...
    answers.get(TotalConsiderationOfLinkedTransactionPage).map {
      answer =>
        ...
        SummaryListRowViewModel(
          key     = "transaction.totalConsiderationOfLinkedTransaction.checkYourAnswersLabel",
          value   = ValueViewModel(HtmlFormat.escape(s"£$answer").toString),
          actions = Seq(
            ActionItemViewModel("site.change", controllers.transaction.routes.TotalConsiderationOfLinkedTransactionController.onPageLoad(CheckMode).url)
