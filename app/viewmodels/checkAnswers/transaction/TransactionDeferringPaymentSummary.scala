...
object TransactionDeferringPaymentSummary  {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): SummaryListRow = {
    val changeRoute = controllers.transaction.routes.TransactionDeferringPaymentController.onPageLoad(CheckMode).url
    val label = messages("transaction.deferringPayment.checkYourAnswersLabel")
    answers.get(TransactionDeferringPaymentPage).map {
      answer =>
      ...
      val value = ValueViewModel(
        HtmlContent(
          s"""<a href="$changeRoute" class="govuk-link">${messages("transaction.deferringPayment.missing")}</a>""")
      )
      SummaryListRowViewModel(
        key = label,
        value = value
