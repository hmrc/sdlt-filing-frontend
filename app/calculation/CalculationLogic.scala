...
object CalculationLogic {
  ...
  def calcTaxWithMax(amount: BigDecimal, rate: BigDecimal, max: Option[BigDecimal]): BigDecimal = {
    calcTax(optionalMax(amount, max), rate)
  }

  def optionalMax(amount: BigDecimal, oMax: Option[BigDecimal]): BigDecimal = {
    oMax.map { max =>
