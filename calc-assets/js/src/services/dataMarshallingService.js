  ...
  app.service('dataMarshallingService', function(){
    ...
    var constructCalculationRequest = function(data){
      ...
      if(data.propertyType === 'Residential') {
        ...
        if(data.individual === 'Yes' && data.twoOrMoreProperties === 'No' &&
            ...
            (validator.effectiveDateIsBeforeJuly2020(data.effectiveDate) || validator.effectiveDateIsAfterJune2021(data.effectiveDate))) {
    ...
    function constructFirstTimeBuyerDetails(data) {
      if(data.ownedOtherProperties === "No" && data.mainResidence === "Yes") {
        if(data.sharedOwnership === 'Yes' && data.currentValue === 'atOrBelowThreshold'){
          return "Yes";
        }else if (data.currentValue === 'aboveThreshold') {
            return "No";
        }else{
           return "Yes";
        }
