...
class CalculationControllerLeaseholdApril21ISpec extends BaseSpec with GuiceOneServerPerSuite {
  ...
  "Hitting the /calculate route" should {
    "return a 200 and valid result for leasehold property type" when {
      ...
      "the effective date is before 17/03/2016" when {

        "Non-residential: premium of 100000" in{
          def request: WSResponse = ws.url(
              calculateUrl)
            .post(
              Json.parse(
        ...
        "Non-residential: premium of 500000" in{
          def request: WSResponse = ws.url(
              calculateUrl)
            .post(
              Json.parse(
        ...
        "Mixed: premium of 1000000 with NPV > 150K" in {
          def request: WSResponse = ws.url(
              calculateUrl)
            .post(
              Json.parse(
